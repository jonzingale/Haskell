...
prop_sθ_sφ_x_no_z = do
  ...
  return $ (eBall 12) eval (rs * sqrt (1 + (2/sqrt 5)**2))
  where
...
prop_lθ_sφ_xz = do
  ...
  let eval = sum [ seg | (_, seg) <- takeWhile (stopCond s) ijkSeg]
  return $ (eBall 12) eval (rs * sqrt 34 / 4)
  where
    stopCond s ((x,y,z), _) = x>=0 && y<s && z<s
...
prop_lθ_lφ_xz = do
  ...
  let eval = sum [ seg | (_, seg) <- takeWhile (stopCond s) ijkSeg]
  return $ (eBall 12) eval (rs * sqrt 881 / 20)
  where
...
prop_arbitrary_coords = do
  ...
  let eval = sum [ seg | (_, seg) <- takeWhile (stopCond s) ijkSeg]
  return $ (eBall 12) eval (1 * sqrt 5833 / 60)
  where
